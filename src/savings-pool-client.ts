...
export default class SavingsPoolClient {
  ...
  public constructor(
    ...
    public readonly savingsPoolAddress: Address,
  ) {
   ...
   * @returns The operation hash
   */
  public async withdraw(lpTokenAmount: BigNumber): Promise<TransactionOperation | TransactionWalletOperation> {
    const liquidityPoolContract = await this.tezos.wallet.at(this.savingsPoolAddress)
    const sendArgs = { amount: 0, mutez: true }
    return await liquidityPoolContract.methods['withdraw'](lpTokenAmount).send(sendArgs)
  }
}
